...
export function compareSources(cluster) {
  ...
  // Emphasis differences: dimensions led by some sources but absent in others.
  const emphasis = [];
  if (rows.length >= 2) {
    for (const dim of FRAMING_DIMENSIONS) {
      const leads = rows.filter((r) => r.framing[0]?.id === dim.id).map((r) => r.source);
      const ignores = rows.filter((r) => !r.framing.some((f) => f.id === dim.id)).map((r) => r.source);
      if (leads.length > 0 && ignores.length > 0) {
        emphasis.push({
          dimension: dim.label,
          emphasizedBy: [...new Set(leads)],
          absentFrom: [...new Set(ignores)].slice(0, 4),
        });
      }
    }
  }
