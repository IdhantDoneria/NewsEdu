...
import { extractEntities, extractEventTerms, extractKeyNumbers } from './entities.mjs';
...
export function detectChanges(cluster, snapshot) {
  ...
  for (const a of newArticles) {
    ...
    changes.push({
      ...
      why: signal?.why || (numbers.length ? 'New concrete figures update the factual picture.' : 'New actors have entered the story.'),
      type: signal?.type || (numbers.length ? 'New data' : 'New actor'),
      at: a.publishedAt,
      source: a.sourceName,
