...
/**
 * Light stemming so headline variants of one event agree: possessives and
 * common verb/plural suffixes are stripped ("buries"/"buried" → "buri",
 * "iran's" → "iran", "arrives"/"arrived" → "arriv").
 */
function stem(w) {
  let t = w.replace(/'s$/, '');
  if (t.length > 5) t = t.replace(/(?:ing|ed|es)$/, '');
  if (t.length > 4) t = t.replace(/s$/, '');
  return t;
}
...
function articleFeatures(article) {
  // A period between title and summary is a real sentence boundary — without
  // it, a capitalised word ending the title can fuse with a capitalised word
  // starting the summary into a bogus two-word entity ("...in Philippines" +
  // "Heading for..." -> "Philippines Heading").
...
function belongs(feat, cluster) {
  ...
  // Same specific actor set with some headline agreement. Two shared named
  // entities plus two shared (stemmed) title tokens is how divergent
  // headlines of one event look ("arrives safely in UK" vs "smuggled into
  // Britain") without merging unrelated stories that share one country.
  if (sharedEntities >= 2 && sharedTitleTokens >= 2) return true;
...
/** Stemmed significant-token set for a title — shared with briefing dedup. */
export function titleTokenSet(text) {
