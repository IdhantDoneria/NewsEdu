...
/**
 * Runs L1-L9 and returns each layer's signed contribution alongside the
 * clamped L10 total. Both `calculateFinancialScore` (plain integer) and
 * `calculateFinancialScoreDetailed` (score + labeled breakdown, used by the
 * UI's score explanation) derive from this single implementation so the
 * displayed number and its explanation can never drift apart.
 */
function runLayers(articleData) {
  ...
  const add = (key, label, delta) => {
    running += delta;
    layers.push({ key, label, delta: Math.round(delta * 10) / 10 });
  };

  // L1 — source authority baseline
  add('sourceAuthority', 'Source authority', safeLayer(() => sourceAuthorityBaseline(articleData), 40));
  // L2 — Unique tickers × 1.5 (bonus)
  add('tickers', 'Tickers & instruments', safeLayer(() => uniqueTickerCount(text) * 1.5));
  // L3 — Quantitative Data Density (%) × 0.5 (bonus, capped at 25)
  add('density', 'Quantitative data density', safeLayer(() => Math.min(25, quantitativeDataDensity(text) * 0.5)));
  // L4 — Sentiment Deviation (penalty)
  add('sentiment', 'Volatility language', -safeLayer(() => sentimentDeviationPenalty(text)));
  // L5 — Regulatory Verification (+15 flat when matched)
  add('regulatory', 'Regulatory filing', safeLayer(() => regulatoryVerificationBonus(text)));
  // L6 — Macro Relevance × 0.1 (bonus)
  add('macro', 'Macroeconomic relevance', safeLayer(() => macroRelevance(text) * 0.1));
  // L7 — Institutional Flow Signals (+5 flat when matched)
  add('institutional', 'Institutional flow signals', safeLayer(() => institutionalFlowBonus(text)));
  // L8 — Fundamental Metrics Depth × 0.15 (bonus)
  add('fundamentals', 'Fundamental valuation depth', safeLayer(() => fundamentalMetricsDepth(text) * 0.15));
  // L9 — Speculation & Clickbait (penalty)
  add('speculation', 'Speculation & clickbait', -safeLayer(() => speculationClickbaitPenalty(articleData)));

  // L10 — Final Multi-Factor Normalisation
  const score = clampInt(running, 0, 100);
  return { score, layers };
}
...
export function calculateFinancialScore(articleData) {
  ...
  return runLayers(articleData).score;
}

/**
 * Same pipeline, but also returns the signed per-layer contributions that
 * summed (pre-clamp) to the score. Used by the UI to render a breakdown that
 * is guaranteed to match the displayed number.
 */
export function calculateFinancialScoreDetailed(articleData) {
  if (!articleData || typeof articleData !== 'object') return { score: 0, layers: [] };
  return runLayers(articleData);
}
