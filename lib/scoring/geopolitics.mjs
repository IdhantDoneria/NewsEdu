 ...
 * clamped L10 total. Both `calculateGeopoliticalScore` (plain integer) and
 * `calculateGeopoliticalScoreDetailed` (score + labeled breakdown, used by
 * the UI's score explanation) derive from this single implementation so the
...
function runLayers(articleData) {
  ...
  // L1 — source credibility baseline
  add('sourceCredibility', 'Source credibility', safeLayer(() => sourceCredibilityBaseline(articleData), 45));
  // L2 — Fact Match Rate × 0.2 (bonus)
  add('factMatch', 'Verified facts & entities', safeLayer(() => factMatchRate(text) * 0.2));
  // L3 — Temporal Relevance Decay (penalty)
  add('temporalDecay', 'Age decay', -safeLayer(() => temporalDecayPenalty(articleData)));
  // L4 — Sentiment & Bias Neutralisation (penalty)
  add('subjectivity', 'Subjective language', -safeLayer(() => subjectivityPenalty(text)));
  // L5 — Information Density Bonus
  add('density', 'Information density', safeLayer(() => informationDensityBonus(text)));
  // L6 — Network Impact / Citation Bonus
  add('citation', 'Outlet authority', safeLayer(() => citationBonus(articleData)));
  // L7 — Geopolitical Event Relevance × 0.1 (bonus)
  add('eventRelevance', 'Geopolitical relevance', safeLayer(() => geopoliticsRelevance(text) * 0.1));
  // L8 — Cross-Market Impact (secondary adjustment)
  add('crossMarket', 'Cross-border impact', safeLayer(() => crossMarketAdjustment(text)));
  // L9 — Clickbait / Sensationalism (penalty)
  add('clickbait', 'Clickbait & sensationalism', -safeLayer(() => clickbaitPenalty(articleData)));
 ...
 */
export function calculateGeopoliticalScoreDetailed(articleData) {
