...
export function scoreArticle(raw, source, edition, now = Date.now()) {
  ...
  return {
    ...
    // The full source is attached (not only its name) so the 10-layer pipelines
    // in lib/scoring/*.js can read trust/authority for their Layer 1 baselines.
    source: { name: source.name, trust: source.trust, authority: source.authority },
    edition,
    metrics: {
