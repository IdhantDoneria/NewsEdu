...
- [x] In the DSPR website, if the website generation is complete then try to code for react based animations that make the website genuinely very immersive, just do not overdo anything, everything should be made very intricately. The website experience should truly be very energetic, and seem like the best PR team out there.
  ...
  - Enhanced 2026-06-14: Added gold scroll-progress bar (ScrollProgressBar), radial cursor glow (CursorGlow), canvas floating-particle field (FloatingParticles), mouse-tracking parallax on Hero orbs (useMouseParallax hook), count-up number animations for stats (AnimatedCounter), 3D perspective tilt on About pillars / Case Study cards / Team cards (useTilt hook), shimmer sweep line on CaseStudy card hover, marquee pause-on-hover with gold text highlight, and line-draw reveal on pre-title separators. Build green.
